...
test("labels uploaded documents as not yet retrievable", () => {
  ...
  assert.equal(getDocumentStatusLabel("chunking"), "分段创建中")
  assert.equal(getDocumentAvailabilityLabel("chunking"), "处理中")
  assert.equal(getDocumentStatusLabel("chunked"), "已分段，待向量化")
  assert.equal(getDocumentStatusLabel("embedding"), "向量生成中")
  ...
  assert.equal(getDocumentStatusLabel("deleting"), "删除清理中，可重试")
  assert.equal(getDocumentAvailabilityLabel("deleting"), "不可用")
  assert.equal(isDocumentRetrievable("deleting"), false)
})
